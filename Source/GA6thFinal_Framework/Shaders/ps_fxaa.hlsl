...
ConstantBuffer<FXAAData> fxaaData;
...
float4 ps_main(PSInput input) : SV_Target
{
    ...
    // offset
    float2 offset = fxaaData.InverseResolution;
    ...
    float threshold = max(fxaaData.QualityEdgeDetectionThreshold * maxLum,
                          fxaaData.QualityMinimumEdgeThreshold);
    ...
    if (lumRange < threshold)
    {
        ...
        return float4(lerp(centerColor, (northColor + southColor + eastColor + westColor) * 0.25f, subPixel * fxaaData.QualitySubpixel), 1);
    }
    ...
    float2 step = edgeDir * fxaaData.InverseResolution;
    ...
    float3 finalColor = lerp(centerColor, blendColor, fxaaData.QualitySubpixel);
