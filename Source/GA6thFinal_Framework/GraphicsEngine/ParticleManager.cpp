...
void ParticleManager::CreateDescriptors()
{
    ...
    Global::device->GetDevice()->CreateConstantBufferView(&cbvDesc, handle);
    ...
    Global::device->GetDevice()->CreateShaderResourceView(_particleInputBuffer.Get(), &particleInputSrvDesc, handle);
    ...
    Global::device->GetDevice()->CreateShaderResourceView(_emitterInfoBuffer.Get(), &emitterInfoSrvDesc, handle);
    ...
    Global::device->GetDevice()->CreateUnorderedAccessView(_particleOutputBuffer.Get(), nullptr, &particleOutputUavDesc,
                                                    handle);
}
...
void ParticleManager::DispatchParticleCompute(float deltaTime)
{
    ...
    _computeCommandList->Close();
    Global::commandController->ExecuteCommand(CommandQueueType::COMPUTE_QUEUE, _computeCommandList.Get());
