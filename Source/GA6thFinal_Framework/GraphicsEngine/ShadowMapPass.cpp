...
void ShadowMapPass::DrawMeshes(ID3D12GraphicsCommandList* commandList, int shaderType, MeshType meshType, int cascadeIndex)
{
    ...
    for (auto& [mesh, instanceID, customDepth] : _renderDatas[meshType])
    {
        ...
        {
        case STATIC_MESH:
            commandList->SetGraphicsRoot32BitConstants(_fxStaticShadow.GetRootParameterIndex("bit32_5_shadowObjectData"), 4, parameter, 0);
            break;

        case SKELETAL_MESH:
            commandList->SetGraphicsRoot32BitConstants(_fxSkeletalShadow.GetRootParameterIndex("bit32_5_shadowObjectData"), 4, parameter, 0);
            break;
        }
