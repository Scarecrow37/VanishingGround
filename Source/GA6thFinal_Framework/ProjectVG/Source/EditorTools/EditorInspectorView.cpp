...
void  EditorInspectorView::OnFrame()
{
    ...
    ImGuiHelper::AlignedText("asd", ImGuiHelper::CENTER, 1.5f);

    ImGui::TextUnformatted("\xef\x85\x9b"); // FILE 아이콘 (FontAwesome 6)
    ImFont* font = ImGui::GetFont();
    ImGui::Text("Font Debug: %s", font->GetDebugName());
    ImGui::Text("Test: \xef\x80\x95 \xef\x89\x83 \xef\x83\x9b"); // Home, 톱니바퀴, 폴더 아이콘

    ImGui::SameLine();
    ImGuiHelper::LoadingSpinner();

    ImGuiHelper::Separator(15.0f, 0.0f);
    ...
    if (ImGui::TreeNode("Reorderable, hideable, with headers"))
    {
        ...
        ImGui::CheckboxFlags("ImGuiTableFlags_NoBordersInBodyUntilResize", &flags, ImGuiTableFlags_NoBordersInBodyUntilResize); ImGui::SameLine(); ImGuiHelper::TooltipMarker("Disable vertical borders in columns Body until hovered for resize (borders will always appear in Headers)");
