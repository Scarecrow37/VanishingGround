...
void blueprint::OnStart()
{
    ...
    _links.push_back(Link(GetNextLinkId(), _nodes[5].Outputs[0].ID, _nodes[6].Inputs[0].ID));
    _links.push_back(Link(GetNextLinkId(), _nodes[5].Outputs[0].ID, _nodes[7].Inputs[0].ID));
...
# endif
    ...
    ed::Begin("Node editor");
    {
        ...
        util::BlueprintNodeBuilder builder(_headerBackground, 64, 64);
...
void blueprint::DrawPinIcon(const Pin& pin, bool connected, int alpha)
{
    ...
    ax::Widgets::Icon(ImVec2(static_cast<float>(_pinIconSize),
        static_cast<float>(_pinIconSize)),
...
void blueprint::ShowLeftPane(float paneWidth)
{
    ...
    ImGui::EndHorizontal();
    ImGui::Checkbox("Show Ordinals", &_showOrdinals);
    ...
    /*int saveIconWidth = GetTextureWidth(_saveIcon);
    int saveIconHeight = GetTextureWidth(_saveIcon);
    int restoreIconWidth = GetTextureWidth(_restoreIcon);
    int restoreIconHeight = GetTextureWidth(_restoreIcon);*/
...
{
    auto it = _nodeTouchTime.find(id);
    if (it != _nodeTouchTime.end() && it->second > 0.0f)
...
{
    _nodeTouchTime[id] = _touchTime;
}

void blueprint::UpdateTouch()
{
    const auto deltaTime = ImGui::GetIO().DeltaTime;
    for (auto& entry : _nodeTouchTime)
    {
...
{
    _nodes.emplace_back(GetNextId(), "InputAction Fire", ImColor(255, 128, 128));
    _nodes.back().Outputs.emplace_back(GetNextId(), "", PinType::Delegate);
    _nodes.back().Outputs.emplace_back(GetNextId(), "Pressed", PinType::Flow);
    _nodes.back().Outputs.emplace_back(GetNextId(), "Released", PinType::Flow);
...
{
    _nodes.emplace_back(GetNextId(), "Branch");
    _nodes.back().Inputs.emplace_back(GetNextId(), "", PinType::Flow);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Condition", PinType::Bool);
    _nodes.back().Outputs.emplace_back(GetNextId(), "True", PinType::Flow);
    _nodes.back().Outputs.emplace_back(GetNextId(), "False", PinType::Flow);
...
{
    _nodes.emplace_back(GetNextId(), "Do N");
    _nodes.back().Inputs.emplace_back(GetNextId(), "Enter", PinType::Flow);
    _nodes.back().Inputs.emplace_back(GetNextId(), "N", PinType::Int);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Reset", PinType::Flow);
    _nodes.back().Outputs.emplace_back(GetNextId(), "Exit", PinType::Flow);
    _nodes.back().Outputs.emplace_back(GetNextId(), "Counter", PinType::Int);
...
{
    _nodes.emplace_back(GetNextId(), "OutputAction");
    _nodes.back().Inputs.emplace_back(GetNextId(), "Sample", PinType::Float);
    _nodes.back().Outputs.emplace_back(GetNextId(), "Condition", PinType::Bool);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Event", PinType::Delegate);
...
{
    _nodes.emplace_back(GetNextId(), "Print String");
    _nodes.back().Inputs.emplace_back(GetNextId(), "", PinType::Flow);
    _nodes.back().Inputs.emplace_back(GetNextId(), "In String", PinType::String);
...
{
    _nodes.emplace_back(GetNextId(), "", ImColor(128, 195, 248));
    _nodes.back().Type = NodeType::Simple;
    _nodes.back().Outputs.emplace_back(GetNextId(), "Message", PinType::String);
...
{
    _nodes.emplace_back(GetNextId(), "Set Timer", ImColor(128, 195, 248));
    _nodes.back().Inputs.emplace_back(GetNextId(), "", PinType::Flow);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Object", PinType::Object);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Function Name", PinType::Function);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Time", PinType::Float);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Looping", PinType::Bool);
    _nodes.back().Outputs.emplace_back(GetNextId(), "", PinType::Flow);
...
{
    _nodes.emplace_back(GetNextId(), "o.O", ImColor(128, 195, 248));
    _nodes.back().Type = NodeType::Simple;
    _nodes.back().Inputs.emplace_back(GetNextId(), "", PinType::Float);
    _nodes.back().Outputs.emplace_back(GetNextId(), "", PinType::Float);
    _nodes.back().Outputs.emplace_back(GetNextId(), "", PinType::Float);
...
{
    _nodes.emplace_back(GetNextId(), "Single Line Trace by Channel", ImColor(255, 128, 64));
    _nodes.back().Inputs.emplace_back(GetNextId(), "", PinType::Flow);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Start", PinType::Flow);
    _nodes.back().Inputs.emplace_back(GetNextId(), "End", PinType::Int);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Trace Channel", PinType::Float);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Trace Complex", PinType::Bool);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Actors to Ignore", PinType::Int);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Draw Debug Type", PinType::Bool);
    _nodes.back().Inputs.emplace_back(GetNextId(), "Ignore Self", PinType::Bool);
    _nodes.back().Outputs.emplace_back(GetNextId(), "", PinType::Flow);
    _nodes.back().Outputs.emplace_back(GetNextId(), "Out Hit", PinType::Float);
    _nodes.back().Outputs.emplace_back(GetNextId(), "Return Value", PinType::Bool);
...
{
    _nodes.emplace_back(GetNextId(), "Sequence");
...
{
    _nodes.emplace_back(GetNextId(), "Random Wait");
    _nodes.back().Type = NodeType::Tree;
    _nodes.back().Inputs.emplace_back(GetNextId(), "", PinType::Flow);
...
{
    _nodes.emplace_back(GetNextId(), "Test Comment");
...
{
    _nodes.emplace_back(GetNextId(), "Transform");
    _nodes.back().Type = NodeType::Houdini;
    _nodes.back().Inputs.emplace_back(GetNextId(), "", PinType::Flow);
    _nodes.back().Outputs.emplace_back(GetNextId(), "", PinType::Flow);
...
{
    _nodes.emplace_back(GetNextId(), "Group");
    _nodes.back().Type = NodeType::Houdini;
    _nodes.back().Inputs.emplace_back(GetNextId(), "", PinType::Flow);
    _nodes.back().Inputs.emplace_back(GetNextId(), "", PinType::Flow);
    _nodes.back().Outputs.emplace_back(GetNextId(), "", PinType::Flow);
...
ImRect blueprint::ImGui_GetItemRect()
{
    return ImRect(ImGui::GetItemRectMin(), ImGui::GetItemRectMax());
}

ImRect blueprint::ImRect_Expanded(const ImRect& rect, float x, float y)
{
    auto result = rect;
    result.Min.x -= x;
    result.Min.y -= y;
...
bool blueprint::Splitter(bool split_vertically, float thickness, float* size1, float* size2, float min_size1, float min_size2, float splitter_long_axis_size)
{
