...
public:
	...
	float GetEngineTime();
	D3D12_VIEWPORT GetMainViewport() const { return _mainViewport; }
...
public:
	...
	void RegisterCommand(ID3D12CommandList* commandList);
	void ExecuteCommand();
	void ResetCommands();

	HRESULT UpdateBuffer(ComPtr<ID3D12Resource>& buffer, void* data, UINT size);
	/**
	* @brief !!!!call after set pso!!!!
	* @param flag
	* @param color
	* @param depth
	* @param stencil
	...
	*/
	...
	HRESULT CreateVertexBuffer(void* data, UINT size, UINT stride, ComPtr<ID3D12Resource>& buffer, D3D12_VERTEX_BUFFER_VIEW& view);
	HRESULT CreateIndexBuffer(void* data, UINT size, DXGI_FORMAT format, ComPtr<ID3D12Resource>& buffer, D3D12_INDEX_BUFFER_VIEW& view);
